        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('nongchanpinCollection','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('nongchanpinCollection','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('nongchanpinCollection','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('nongchanpinCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/lelenongchanpinxiaoshou/upload/nongchanpinCollectionMuBan.xls"
                        >批量导入农产品收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('nongchanpinCollection','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="nongchanpinCollectionUploadSuccess"
                                :on-error="nongchanpinCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('nongchanpinCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入农产品收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('nongchanpinCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "nongchanpinCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('nongchanpinCollection','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="nongchanpinCollectionTypes"
                                      header-align="center"
                                      label="类型">
                        <template slot-scope="scope">
                            {{scope.row.nongchanpinCollectionValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('nongchanpinCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('nongchanpinCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('nongchanpinCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                nongchanpinId : null,
                yonghuId : null,
                nongchanpinCollectionTypes : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'nongchanpin_collection',//当前表表名,
                        sumColum : 'nongchanpin_collection_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'nongchanpin_collection_name',//分组字符串字段
                        // types : 'nongchanpin_collection_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "nongchanpin_collection_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['nongchanpinCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "nongchanpinCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            deleteHandler(id) {
                ...
                this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
                    ...
                    this.$http({
                        url: "nongchanpinCollection/delete",
                        method: "post",
                        data: ids
                    }).then(({data}) => {
                        if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            nongchanpinCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "nongchanpinCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            nongchanpinCollectionUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "nongchanpinCollection/update",
                        method: "post",
                        data: {
                            id:id,
//                            nongchanpinCollectionTypes:nongchanpinCollectionTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
