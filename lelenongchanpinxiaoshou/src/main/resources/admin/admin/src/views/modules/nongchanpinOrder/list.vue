        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('nongchanpinOrder','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('nongchanpinOrder','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('nongchanpinOrder','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('nongchanpinOrder','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/lelenongchanpinxiaoshou/upload/nongchanpinOrderMuBan.xls"
                        >批量导入农产品订单数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('nongchanpinOrder','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="nongchanpinOrderUploadSuccess"
                                :on-error="nongchanpinOrderUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('nongchanpinOrder','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入农产品订单数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('nongchanpinOrder','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "nongchanpinOrder.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('nongchanpinOrder','查看')"
                          :data="dataList"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="nongchanpinPhoto"
                               header-align="center"
                               width="200"
                               label="农产品照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.nongchanpinPhoto">
                                <img :src="$base.url+scope.row.nongchanpinPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="nongchanpinTypes"
                                      header-align="center"
                                      label="农产品类型">
                        <template slot-scope="scope">
                            {{scope.row.nongchanpinValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="yonghuName"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongchanpinOrderUuidNumber"
                                   header-align="center"
                                   label="订单号">
                        <template slot-scope="scope">
                            {{scope.row.nongchanpinOrderUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="buyNumber"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongchanpinOrderTruePrice"
                                   header-align="center"
                                   label="实付价格">
                        <template slot-scope="scope">
                            {{scope.row.nongchanpinOrderTruePrice}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongchanpinOrderCourierName"
                                   header-align="center"
                                   label="快递公司">
                        <template slot-scope="scope">
                            {{scope.row.nongchanpinOrderCourierName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongchanpinOrderCourierNumber"
                                   header-align="center"
                                   label="订单快递单号">
                        <template slot-scope="scope">
                            {{scope.row.nongchanpinOrderCourierNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="nongchanpinOrderTypes"
                                      header-align="center"
                                      label="订单类型">
                        <template slot-scope="scope">
                            {{scope.row.nongchanpinOrderValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('nongchanpinOrder','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('nongchanpinOrder','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('nongchanpinOrder','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
            <el-button v-if="isAuth('nongchanpinOrder','订单') && scope.row.nongchanpinOrderTypes == 101 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-sold-out" size="mini" @click="refund(scope.row.id)">退款</el-button>
            <el-button v-if="isAuth('nongchanpinOrder','订单') && scope.row.nongchanpinOrderTypes == 104 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="openCommentModel(scope.row.id)">评价</el-button>
            <el-button v-if="isAuth('nongchanpinOrder','订单') && scope.row.nongchanpinOrderTypes == 101
&& sessionTable=='shangjia' && userId==scope.row.nongchanpinShangjiaId" type="primary" icon="el-icon-reading" size="mini"
                        @click="openDeliver(scope.row.id)"
            >发货</el-button>
            <el-button v-if="isAuth('nongchanpinOrder','订单') && scope.row.nongchanpinOrderTypes == 103 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="receiving(scope.row.id)">收货</el-button>
                ...
                <el-dialog title="发货" :visible.sync="deliverVisible" width="30%">
                    <el-form>
                        <el-form-item label="快递公司">
                            <el-input v-model="nongchanpinOrderCourierName" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="订单快递单号">
                            <el-input v-model="nongchanpinOrderCourierNumber" autocomplete="off"></el-input>
                        </el-form-item>
                    </el-form>
                    <span slot="footer" class="dialog-footer">
        ...
        <el-dialog title="统计报表" :visible.sync="chartVisiable" width="80%">
            <el-date-picker v-model="echartsDate" type="month" placeholder="选择年"> </el-date-picker>
...
<script>
    ...
    export default {
        ...
        return {
            ...
            deliverId:null,
			//快递单号
            nongchanpinOrderCourierNumber:null,
			//快递公司
            nongchanpinOrderCourierName:null,
			//模态框状态
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                nongchanpinOrderUuidNumber : null,
                addressId : null,
                nongchanpinId : null,
                yonghuId : null,
                buyNumber : null,
                nongchanpinOrderTruePrice : null,
                nongchanpinOrderCourierName : null,
                nongchanpinOrderCourierNumber : null,
                nongchanpinOrderTypes : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '余额': 'newMoney',
                //本表字段
                     '订单号': "nongchanpinOrderUuidNumber",
                     '购买数量': "buyNumber",
                     '实付价格': "nongchanpinOrderTruePrice",
                     '快递公司': "nongchanpinOrderCourierName",
                     '订单快递单号': "nongchanpinOrderCourierNumber",
                     '订单类型': "nongchanpinOrderValue",
                     '订单创建时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'nongchanpin_order',//当前表表名,
                        sumColum : 'nongchanpin_order_true_price', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'nongchanpin_order_name',//分组字符串字段
                        // types : 'nongchanpin_order_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "nongchanpin_order_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['nongchanpinOrderDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "nongchanpinOrder/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            nongchanpinOrderUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "nongchanpinOrder/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            nongchanpinOrderUploadError(data){
			...
			refund(id){
				this.$confirm(`确定要退款吗?`, "提示", {
					...
					this.$http({
						url: "nongchanpinOrder/refund?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							this.$message({
			...
			openDeliver(id){
				//清空快递单号
				this.nongchanpinOrderCourierNumber = null;
				//快递公司
				this.nongchanpinOrderCourierName = null;
				//赋值
				this.deliverId = id;
				//打开模态框
			...
			deliver(){
				let _this = this;
				if(this.nongchanpinOrderCourierNumber == ""
					|| this.nongchanpinOrderCourierNumber == null
					|| this.nongchanpinOrderCourierNumber == "null"){
						return this.$message.error("订单快递单号不能为空");
				}
				if(this.nongchanpinOrderCourierName == ""
					|| this.nongchanpinOrderCourierName == null
					|| this.nongchanpinOrderCourierName == "null"){
				...
				_this.$confirm(`确定要发货吗?`, "提示", {
					...
					_this.$http({
						url: "nongchanpinOrder/deliver?id="+id+"&nongchanpinOrderCourierNumber="+_this.nongchanpinOrderCourierNumber+"&nongchanpinOrderCourierName="+_this.nongchanpinOrderCourierName,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
			...
			receiving(id){
				this.$confirm(`确定要收货吗?`, "提示", {
					...
					this.$http({
						url: "nongchanpinOrder/receiving?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							this.$message({
			...
			commentback(id){
				...
				_this.$confirm(`确定要评价吗?`, "提示", {
					...
					_this.$http({
						url: "nongchanpinOrder/commentback?id="+_this.commentbackId+"&commentbackText="+_this.commentbackContent+"&nongchanpinCommentbackPingfenNumber="+_this.nongchanpinCommentbackPingfenNumber,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
								...
								onClose: () => {
									...
									//情况评分
									_this.nongchanpinCommentbackPingfenNumber = null
								}
							});
						} else {
							//清空操作数据id
